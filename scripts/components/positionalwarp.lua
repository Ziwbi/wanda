...
local PositionalWarp = Class(function(self, inst)
	...
	if self.inst.components.driver ~= nil then
		self.inst:ListenForEvent("mountboat", function(inst)
			self:EnableMarker(false)
			self:SetPaused(true)
		end)

		self.inst:ListenForEvent("dismountboat", function(inst)
			if inst.components.inventory:Has("pocketwatch_warp", 1) then
				self:EnableMarker(true)
			end
			local fn
			fn = function(_inst)
				self:SetPaused(false)
				_inst:RemoveEventCallback("animqueueover", fn)
			end
			inst:ListenForEvent("animqueueover", fn)
		end)
	end

	if GetWorld().components.interiorspawner ~= nil then
		GetWorld():ListenForEvent("exitinterior", function(...)
			self:EnableMarker(false)
			self:SetPaused(true)
		end)

		GetWorld():ListenForEvent("enterinterior", function(...)
			if self.inst.components.inventory:Has("pocketwatch_warp", 1) then
				self:EnableMarker(true)
			end
			local fn
			fn = function(...)
				self:SetPaused(false)
				self.inst:RemoveEventCallback("animqueueover", fn)
			end
...
function PositionalWarp:SetPaused(paused)
	self.paused = paused
end
...
function PositionalWarp:OnLoad(data)
	if data ~= nil and self.inst.migration == nil then -- dont save/load across worlds
