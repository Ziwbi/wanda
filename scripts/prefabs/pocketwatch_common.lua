...
local function recallmarker_ondropped(inst)
	if inst._owner ~= nil then
		inst:RemoveEventCallback("respawn", inst._respawn_handler, inst._owner)
		inst._owner = nil
	end
...
local function recallmarker_create(inst)
	...
	if inst.components.recallmark:IsMarked() then
		...
		if owner ~= nil then
			local x, y, z = inst.components.recallmark:GetMarkedPosition() 
			-- if this returns nil, then it is not marked or the marked poistion is in another shard
			if owner ~= nil and x ~= nil then
				...
				if inst._owner == nil then
					inst._owner = owner
					inst:ListenForEvent("respawn", inst._respawn_handler, owner)
				end
			end
		end
	end
...
local function RecallMarkable_GetStatus(inst, viewer)
	return (inst.components.rechargeable ~= nil and not inst.components.rechargeable:IsCharged()) and "RECHARGING"
			or viewer:HasTag("pocketwatchcaster") 
			and ((inst.components.recallmark ~= nil and inst.components.recallmark:IsMarked()) 
			and (inst.components.recallmark.level_mode == SaveGameIndex:GetCurrentMode() and "MARKED_SAMESHARD" or "MARKED_DIFFERENTSHARD") or "UNMARKED")
			or nil
end
...
local function MakeRecallMarkable(inst)
	...
	inst._respawn_handler = function() recallmarker_create(inst) end
